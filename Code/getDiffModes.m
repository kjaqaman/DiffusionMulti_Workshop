...
if ~isempty(mask) && any(mask(:)==0)
    ...
    for iTrack = 1 : numTracks
        ...
        %if the mean position falls outside the image (due to shifting)
        %make meanPos be at the border of the image
        meanPosX = max(min(meanPosX,imgSize(2)),1);
        meanPosY = max(min(meanPosY,imgSize(1)),1);
        %if mean displacement of track is inside mask keep it
